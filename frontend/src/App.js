                    ...
                    {jobMatchResults.map((job, i) => (
                      <div key={i} className="job-match-result-card">
                        ...
                        <div className="job-match-result-links">
                          <a href={`https://www.linkedin.com/jobs/search/?keywords=${encodeURIComponent(job.search_keywords || job.title)}${jobSearchFilters.location ? '&location=' + encodeURIComponent(jobSearchFilters.location) : ''}&f_TPR=${jobSearchFilters.posted === '1hr' ? 'r3600' : jobSearchFilters.posted === '24hr' ? 'r86400' : jobSearchFilters.posted === '3days' ? 'r259200' : jobSearchFilters.posted === 'week' ? 'r604800' : jobSearchFilters.posted === 'month' ? 'r2592000' : ''}${jobSearchFilters.experience === 'internship' ? '&f_E=1' : jobSearchFilters.experience === 'entry' ? '&f_E=2' : jobSearchFilters.experience === 'mid' ? '&f_E=3' : jobSearchFilters.experience === 'senior' ? '&f_E=4' : ''}`} target="_blank" rel="noreferrer">🔗 LinkedIn</a>
                          <a href={`https://www.indeed.com/jobs?q=${encodeURIComponent(job.search_keywords || job.title)}${jobSearchFilters.location ? '&l=' + encodeURIComponent(jobSearchFilters.location) : ''}${jobSearchFilters.posted === '1hr' ? '&fromage=last' : jobSearchFilters.posted === '24hr' ? '&fromage=1' : jobSearchFilters.posted === '3days' ? '&fromage=3' : jobSearchFilters.posted === 'week' ? '&fromage=7' : jobSearchFilters.posted === 'month' ? '&fromage=30' : ''}${jobSearchFilters.jobType === 'fulltime' ? '&jt=fulltime' : jobSearchFilters.jobType === 'parttime' ? '&jt=parttime' : jobSearchFilters.jobType === 'contract' ? '&jt=contract' : ''}`} target="_blank" rel="noreferrer">🔗 Indeed</a>
